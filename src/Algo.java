...
public class Algo {
	...
	private static String standard(Query q, Network net) {
		...
		for(String evidence: q.evidence) {
			...
			if(parents.contains(evidence.substring(0, evidence.indexOf("=")))) count_parent++;
		}
		if(no_parents==true||count_parent==parents.size()) {
		double result = net.vars.get(wanted_var).getProb(Arrays.asList(q.evidence), q.wanted.substring( q.wanted.indexOf("=")+1));
System.out.println( result+","+0+","+0);
		...
		for (String var_name : hidden_vars_names) { // 
			...
			all_combination.add(a); // first, all_combination is list of list that any entire list is the values of some variable
		}
		all_combination = product(all_combination); // after 'product' function , all_combination is list of list that every entire list is possible combination of hidden variables values

		for (List<String> combination : all_combination) { // add the evidence to combination
			for (String e : q.evidence)
				if(!e.equals("none"))
				combination.add(e); //add evidence and wanted variables to every list and start compute every list to every value of wanted variable
		...
		double mone = 0;
		double mechane = 0;
		for (String wanted_var_value : net.vars.get(wanted_var).values) { // for normalization we check for every possible value of wanted variable
			double b = 0;
			...
			for (List<String> combination : all_combination) {
				...
				for (String var_to_comute : combination) {
					double val = net.vars.get(var_to_comute.substring(0, var_to_comute.indexOf("=")))
							.getProb(combination, var_to_comute.substring(var_to_comute.indexOf("=") + 1)); // get probability of the variable value when his parent are given on the combination
					;
					c = c * val;
					count_mult++;
				}
				...
				combination.remove(combination.size() - 1); // change wanted variable value - remove the old one and in the head of the loop we put new one
			}
			mechane += b;
		...
		System.out.println("ans:" + (mone / mechane)+","+count_plus+","+count_mult);
		return null;
	}
